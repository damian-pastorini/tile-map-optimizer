...
class TileExtruder
{
    ...
    {
        let {data: srcRaw, info: srcInfo} = await sharp(inputPath).ensureAlpha()
            .raw()
        ...
        if(!Number.isInteger(cols) || !Number.isInteger(rows)){
            Logger.critical(
                'TileExtruder: non-integer number of rows or cols found.',
                {srcWidth, srcHeight, tw, th, margin, spacing, cols, rows}
            );
            return false;
        }
