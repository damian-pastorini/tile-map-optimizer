...
const { FileHandler, Logger, sc, ErrorManager} = require('@reldens/utils');
...
class TileMapOptimizer
{
    ...
    {
        let totalColumns = Math.floor(tileSet.width / (this.tileWidth + tileSet.spacing));
        let totalRows = Math.floor(tileSet.height / (this.tileHeight + tileSet.spacing));
        let tilesCounter = 0;
        let result = false;
    ...
    async createNewJSON()
    {
        ...
        for(let i of Object.keys(this.tileSetData)){
            ...
            for(let tile of tileset.tilesData){
                let newTileData = {
                    id: this.newImagesPositions[tileset.first + tile.id] - 1
                };
                if(tile.animation){
                    let animation = [];
    ...
    async resizeTileset()
    {
        ...
        let json = Object.assign({}, this.newJSON);
